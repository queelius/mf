...
| `packages` | `content/packages` | Package content generation |
...
**What `mf` generates** (`packages/generator.py`):
...
registry: "pypi"              # pypi, cran
latest_version: "1.2.3"
install_command: "pip install package-name"
registry_url: "https://pypi.org/project/package-name/"
downloads: 50000
license: "MIT"
featured: true
tags: [statistics, python]
linked_project: "/projects/project-slug/"
aliases: []
```

Package pages are leaf bundles at `content/packages/{name}/index.md`.

**What the theme must provide** (`layouts/packages/`):

The theme needs a `single.html` layout for individual package pages that renders registry metadata, install instructions, and links. A `list.html` layout should show all packages as a browsable grid or list with filtering by registry and tags.

Fields consumed:
- `registry` — package registry source (pypi, cran)
- `latest_version` — displayed version string
- `install_command` — copy-paste install instruction
- `registry_url` — link to package on registry
- `downloads` — download count (for badges/stats)
- `license` — license badge
- `featured` — highlight in listings
- `linked_project` — link to associated mf project page
- `tags` — tag badges

**Registry adapters** follow a plugin pattern. Built-in adapters ship in `src/mf/packages/registries/` (pypi.py, cran.py). Users can override or extend by placing scripts in `.mf/registries/`. Each adapter module exposes a module-level `adapter` instance satisfying the `RegistryAdapter` protocol.
...
3. **Content sections**: `content/post/`, `content/papers/`, `content/projects/`, `content/publications/`, `content/series/`, `content/packages/` (and optionally `content/writing/`, `content/research/`)
...
8. **Section layouts**: `layouts/{papers,projects,publications,series,packages,post,writing,research}/` with list and single templates
9. **Packages layout**: `layouts/packages/list.html` and `single.html` for package registry pages (generated by `mf packages generate`)
